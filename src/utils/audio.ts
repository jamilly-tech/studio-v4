...
function ipcSilenceIntervalsToCuts(
  intervals: { start: number; end: number; duration: number }[]
): CleanCutPause[] {
  return intervals.map((iv, idx) => {
    const isLong = iv.duration > 0.9;
    const mm = Math.floor(iv.start / 60);
    const ss = Math.floor(iv.start % 60);
    return {
      id: `cut-ffmpeg-${idx + 1}`,
      label: `Pausa ${idx + 1}`,
      ...
      startTime: iv.start,
      endTime: iv.end,
      width: Math.max(56, Math.min(180, iv.duration * 38)),
      color: isLong ? "#52cc5a" : "#ffc02a",
      score: isLong ? 90 : 65,
      reason: isLong ? "Pausa longa — silêncio real" : "Pausa curta — revisar",
      action: (isLong ? "cut" : "review") as "cut" | "review",
    };
  });
}

export async function analyzeAudioForCleanCuts(file: File | string): Promise<{ cuts: CleanCutPause[]; totalDuration: number }> {
  // Preferir backend FFmpeg (Electron) — mais estável e preciso
  const studioApi = (window as any).studioV4;
  if (typeof file === "string" && studioApi?.media?.detectSilence) {
    const res = await studioApi.media.detectSilence(file);
    if (!res.error && res.intervals.length >= 0) {
      return { cuts: ipcSilenceIntervalsToCuts(res.intervals), totalDuration: res.totalDuration };
    }
  }

  // Fallback: análise via Web Audio API (browser / sem Electron)
