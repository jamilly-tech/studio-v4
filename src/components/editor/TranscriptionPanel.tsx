...
export function TranscriptionPanel({ assets, selectedAssetId, onSeek, onCaptionsGenerated, onApplyCuts }: TranscriptionPanelProps) {
  ...
  function applyDeletions() {
    ...
    // Corta os takes repetidos marcados diretamente na timeline
    const asset = assets.find((a) => a.id === selectedAssetId);
    if (onApplyCuts && asset && toDelete.size > 0) {
      const totalDuration = asset.metadata?.durationSeconds ?? 0;
      const cuts: CleanCutPause[] = segments
        .filter((_, i) => toDelete.has(i))
        .map((seg, idx) => ({
          id: `repeat-${idx}`,
          label: `Repetição ${idx + 1}`,
          time: `${seg.start.toFixed(1)}s`,
          startTime: seg.start,
          endTime: seg.end,
          width: 0,
          color: "#ef4444",
      ...
      onApplyCuts(asset.id, cuts, totalDuration);
    }
